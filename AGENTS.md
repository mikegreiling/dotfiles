...
This repository (`~/.local/share/chezmoi`, aliased to `~/Projects/dotfiles`) contains my dotfiles managed by [chezmoi](https://chezmoi.io). This file establishes critical safety guidelines for any coding agent when interacting with chezmoi.

## ⚠️ THIS REPOSITORY IS PUBLIC — NO PLAINTEXT SECRETS, EVER

This repo is mirrored to **public** remotes on both GitHub and GitLab. Anything committed here is world-readable, permanently (history included). Therefore:

- **NEVER commit plaintext credentials of any kind**: API tokens, OAuth access/refresh tokens, client secrets, passwords, cookies, session IDs, private keys, webhook URLs with embedded secrets.
- Secrets belong in **age-encrypted files** (the existing `encrypted_*.age` pattern) or outside the repo entirely.
- **Before `chezmoi add`-ing ANY file, inspect it for embedded secrets** — config files frequently inline tokens. Config that references secrets must do so via env-var placeholders (`${VAR}`, `*_env`/`*Env` keys), never literal values.
- Specific standing case: the **mcporter registry** (`~/.config/mcporter/mcporter.json`) is managed here and must stay secret-free (server URLs, public OAuth client IDs, and env placeholders only). The **mcporter credential vault** (`$XDG_DATA_HOME/mcporter/credentials.json`, i.e. `~/.local/share/mcporter/credentials.json`) holds live OAuth tokens and must **NEVER** be added to this repo.
- Avoid managing **high-churn state files** (caches, credential stores, lockfiles, histories) — this repo is for durable configuration only.
- If a secret is ever committed by mistake: treat it as compromised immediately — rotate/revoke the credential first, then scrub history. Removal alone is not remediation on a public repo.
