...
- **Ordered lists in DRAFTS may store as plain text** (observed 2026-07-23, root cause not fully isolated): the same `1.`/`2.` markdown reliably becomes a real `rich_text_list(ordered)` in direct sends (verified via raw blocks, content-independent), but drafts have failed to tokenize it — reproduced twice in `im` (1:1 DM) drafts with a long formatting-heavy first item, while one `mpdm` draft with short items appeared to tokenize. Unresolved confound: channel type vs item length/complexity in the draft converter. Until pinned down, treat ANY numbered list in a draft as at-risk: tell Mike to eyeball the composer and re-apply list formatting if the numbers render as plain text, or restructure to avoid numbered lists, or (with his explicit approval) send directly instead of drafting. Bulleted (`-`) lists in drafts are untested.
