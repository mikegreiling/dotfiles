...
**HARD RULE — AC lands BEFORE the merge, not after.** Every QA-able ticket MUST have valid, QA-facing acceptance criteria on it *before any associated MR is merged*. The forcing mechanism: merging to `main` auto-deploys to dev, and the deploy pipeline's `dev_run_ai_tester` job fires Patrick Spracklen's AI Tester against the ticket **within minutes of the merge**. If the ticket has no AC — or only engineer-facing AC ("emits X to GA4, confirmed received") — the bot posts a "no valid Acceptance Criteria, please manually test" rejection that then sits **stale**: description edits do NOT re-trigger the bot, only Patrick can manually re-run it, and the rejection reads as the current verdict to everyone triaging the ticket. Evidence from 2026-08: GLOB-5046/5047 got AC ~30 minutes *after* the deploy → stale rejections that lingered for two weeks awaiting a re-trigger; GLOB-5127/5128 got AC *at ticket creation* → the bot's first-ever verdict was PASS, same day, zero intervention. Practice: generate the AC block with the `jira-ac-assistant` skill **when the ticket is created** (or at the latest before hitting merge) — observable, browser-verifiable criteria only, with server-side/unobservable verification explicitly fenced out-of-scope pointing at engineering evidence comments. Genuinely non-observable work takes the `nontestable` label instead (see the spike rules below) — never fake observable AC, and never merge first intending to "add AC later."
