...
- **Jira attachments**: neither the Atlassian MCP server nor `acli` can *upload* attachments (verified 2026-07-24; `acli jira workitem attachment` only has `list`/`delete`). Use the `jira-attach` script: `jira-attach [--comment TEXT] ISSUE-KEY FILE [FILE...]` (in `~/.local/bin`, chezmoi-tracked). It reads acli's OAuth bearer token from the macOS login keychain (service `acli`, go-keyring base64+gzip payload) and POSTs multipart to `https://api.atlassian.com/ex/jira/{cloudId}/rest/api/3/issue/{KEY}/attachments` — OAuth tokens only work via the `api.atlassian.com` gateway, never the `bstock.atlassian.net` vanity host. Requires acli to be authenticated; on 401 it self-heals by running a cheap acli command (acli refreshes + re-persists its token) and retrying once. List/delete attachments via `acli jira workitem attachment list --key KEY` / `delete --id ID`.
- **Attaching files "to a comment"**: Jira has no comment-level attachments — files always attach to the issue; comments *embed* them. `jira-attach --comment "text" KEY FILES...` does the whole flow: upload, then post a v2-API comment whose wiki markup (`!name|thumbnail!` for images, `[^name]` for other files) Jira converts server-side into real ADF media nodes — identical to a UI drag-and-drop comment, no media-UUID hackery needed. Caveat: embeds resolve by *filename*, so avoid names that collide with existing attachments on the issue. The MCP `addCommentToJiraIssue` (markdown-in) cannot produce media embeds.
- **Embedding attachments in the description** (or any rich-text field): same wiki-markup mechanism, and **only the raw v2 API converts it** — `PUT /rest/api/2/issue/{KEY}` with `{"fields": {"description": "... !name|thumbnail! ..."}}` via the api.atlassian.com gateway + acli keychain token (see `jira-attach` for the token recipe). Verified 2026-07-24: `acli workitem edit -d` (plain-text mode) stores the markup as **literal text**; MCP `editJiraIssue` with markdown leaves it literal too and turns `![](file)` into a broken external-blob node. Both accept raw ADF, but authoring a `media` node needs the attachment's media UUID, which no attachment API returns — the v2 filename-resolution path is the only practical route. When *editing* an existing description, mind that v2 PUT replaces the whole field and v2 GET returns lossy wiki markup — snapshot the original via v3 (ADF) first if you may need to restore it.
