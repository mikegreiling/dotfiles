...
- Prefer purpose-built tools — `glab`, `gh`, `acli`, and MCP servers — over hand-rolled HTTP. Raw `curl` against an API endpoint is a **last resort**, used only when no CLI, bundled script, or MCP tool covers the operation.
- `glab`/`gh`/`acli` are encouraged and often *preferred* over the equivalent MCP server: they keep large responses out of the context window and run well as background tasks (e.g. polling CI). Use whichever fits the job — just don't drop to `curl` when one of them exists.
- **Jira and Confluence are CLI-covered**: `acli` (OAuth, `--json`) plus the `jira-api` / `confluence-api` wrappers in the `bstock-engineering` skill. The Atlassian MCP server was removed in 2026-07 — if any skill or agent still references `mcp__atlassian__*` tools, load the `atlassian-mcp-shim` skill for the equivalents instead of stopping.
- If an MCP server you actually need is unavailable *and* no CLI covers the service: **STOP** and prompt the user to run `/mcp` to authenticate — don't silently work around it with `curl`.
