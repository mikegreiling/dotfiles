...
- **Prefer Opus over Sonnet for delegation.** Sonnet is very token-inefficient in agentic loops — it typically burns more tokens than Opus on similar prompts, erasing its per-token discount and often costing *more*. Default delegate is the `opus-executor` agent type (pinned to `claude-opus-5`, released 2026-07-24 — near-Fable intelligence at half the price); use `sonnet-executor` only for trivial single-step mechanical tasks or when Opus capacity is exhausted. (Verified 2026-07-24: Claude Code silently resolves the not-yet-recognized `claude-opus-5` pin to `claude-opus-4-8` — an acceptable floor; re-run the model grep after a Claude Code update to confirm it starts routing to Opus 5.)
