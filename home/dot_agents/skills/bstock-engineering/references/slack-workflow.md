...
- **Hand-off flow**: after a verified draft, open the conversation in Mike's Slack app so the draft is waiting in the composer — but **use the `slack://` deep link, NEVER an `https://bstocksolutions.slack.com/...` URL** (per Mike, 2026-08-20: the https form bounces through the browser and fires a "Do you want to allow this website to open Slack.app?" confirmation every time, which he finds obtrusive; he'd rather navigate manually than see it). Recipe (team ID `T0475GQNN` is stable — `slack-api.sh whoami` confirms):
  - channel/DM: `open "slack://channel?team=T0475GQNN&id=<CHANNEL_ID>"`
  - specific message or thread: `open "slack://channel?team=T0475GQNN&id=<CHANNEL_ID>&message=<ts>"` (for a thread draft, pass the thread's parent `thread_ts` so the thread pane opens)
  - The MCP response's `channel_link` and the `Permalink` fields in search results are https URLs — convert them (channel ID + ts are embedded in the permalink: `/archives/<CHANNEL_ID>/p<ts-without-dot>`), don't `open` them as-is. If the deep link ever stops working, fall back to telling Mike where the draft is rather than opening an https URL.
...
**Two scheduling subsystems (discovered 2026-07-28) — mutually invisible**: the MCP tool creates an **internal client "scheduled send"** (`Dr…`-prefixed id, the drafts subsystem) — invisible to `chat.scheduledMessages.list` and unmanageable by `slack-api.sh scheduled-*`; ONLY Mike's client (Drafts & Sent → Scheduled) can see/edit/cancel it. Raw `chat.scheduleMessage` creates a **public-API scheduled message** (`Q…`-prefixed id) — our script CAN list/delete/reschedule it, but it does **NOT appear anywhere in Mike's Slack GUI** (verified: the client's Scheduled view only shows the drafts-subsystem flavor). So the choice is: GUI-manageable (MCP) or script-manageable (raw) — never both. **Default: the raw path** (programmatic control + auditable via `scheduled-list`); always tell Mike the `Q…` ids and fire times, since his client gives him no visibility. The confirmation rule applies identically to both flavors.
...
- **Bare URLs in DRAFTS** (verified 2026-07-29): a bare URL followed by a single newline gets mangled — the auto-linkifier absorbs the newline and the next word into the link token. Use a markdown link instead, or put a blank line after the URL. Sends are unaffected.
...
- **Token location** (since the 2026-08 mcporter migration): the mcporter vault — `$XDG_DATA_HOME/mcporter/credentials.json` when that var is set and absolute (on this machine: `~/.local/share/mcporter/credentials.json`), else `~/.mcporter/credentials.json` — plain 0600 JSON; the Slack entry is the object under `.entries` whose `serverName == "slack"`, token at `.tokens.access_token` (an `xoxe.` rotating user token). `scripts/slack-api.sh`'s `_token()` is the canonical extractor (its `_vault_path()` handles the XDG resolution) — it reads the vault, nudges `mcporter list slack --no-oauth` to refresh when the cached token is stale, and falls back to the legacy Claude Code keychain entry (service `Claude Code-credentials`) until that's decommissioned. Extract in-process; **never echo the token to output or logs**:
  ```bash
  VAULT="${XDG_DATA_HOME:-$HOME/.local/share}/mcporter/credentials.json"; [ -r "$VAULT" ] || VAULT="$HOME/.mcporter/credentials.json"
  TOKEN=$(python3 -c "import json;print(next(e['tokens']['access_token'] for e in json.load(open('$VAULT'))['entries'].values() if e.get('serverName')=='slack'))")
